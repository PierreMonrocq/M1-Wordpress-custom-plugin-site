...
class bbPressModToolsPlugin_bbPress extends bbPressModToolsPlugin {
	...
	public function moderate_content( $content, $post_id ) {
		...
		if ( 'pending' == $post->post_status ) {
			...
			$notice = '<div class="bbp-mt-template-notice">' . __( 'Ce sujet est en attente d\'approbation', $this->plugin_slug ) . '</div>';
	...
	public function user_admin_links() {
		...
		if ( $this->user_can_moderate( get_current_user_id() ) ) {
			...
			if ( isset( $action ) ) {
				...
				echo '<li><a href="' . $nonce_url . '" class="bbp-reply-edit-link">' . __( ucfirst( $action ) . ' Utilisateur', $this->plugin_slug ) . '</a></li>';
			...
			if ( ! in_array( $role, array( 'Keymaster', 'Moderator', 'Senior Moderator' ) ) && $author_id > 0 && ! get_user_meta( $author_id, '_bbp_moderation_flagged', true ) ) {
				...
				$lockdown_action_text = __( 'Surveiller utilisateur',  $this->plugin_slug );

			} elseif ( get_user_meta( $author_id, '_bbp_moderation_flagged', true ) ) {

				$lockdown_action = 'remove_moderation_flag';

				$lockdown_action_text = __( 'Stopper surveillance utilisateur',  $this->plugin_slug );
	...
	public function pending_title( $title, $post_id ) {
		...
		if ( 'pending' == $post->post_status ) {
			...
			return $title . ' ('. __( 'En attente d\'approbation', $this->plugin_slug ) . ')';
		...
		if ( $this->user_can_moderate() ) {
			...
			if ( $child_count > 1 ) {
				...
				return $title . ' ('. $child_count . ' ' . __( 'Réponses en attente d\'approbation', $this->plugin_slug ) . ')';

			} else if ( $child_count == 1 ) {

				return $title . ' ('. $child_count . ' ' . __( 'Réponse en attente d\'approbation', $this->plugin_slug ) . ')';
			...
			if ( in_array( $post->post_type, array( 'topic', 'reply' ) ) && 0 == $post->post_author ) {
				...
				switch( $post->post_type ) {
					...
					case 'topic':
						...
						$message = __('Votre sujet a été enregistré et est maintenant en attente d\'approbation', $this->plugin_slug );

					break;

					case 'reply':

						$message = __('Votre réponse a été enregistrée et est maintenant en attente d\'approbation', $this->plugin_slug );
