...
class bbPressModToolsPlugin_Notifications extends bbPressModToolsPlugin {
	...
	private function handle_post_moderation_notification( $post_id ) {
		...
		$message .= sprintf( __( 'Un nouveau %s a été signalé pour modération: %s', $this->plugin_slug ) . "<br><br>", $post->post_type, '<a href="' . $post_link . '">' . $post_link . '</a>' );

		$message .= sprintf( __( 'Utilisateur: %s', $this->plugin_slug ) . "<br>", $this->get_author_name( $post ) );

		$message .= sprintf( __( 'Sujet: %s', $this->plugin_slug ) . "<br>", $this->get_title( $post ) );

		$message .= sprintf( __( 'Contenu: %s', $this->plugin_slug ) . "<br>", nl2br( $post->post_content ) );
		...
		$this->send_notification( $message, __( 'Marqué ' ) . $post->post_type, $post_id );
	...
	public function handle_report_post_notification( $meta_id, $post_id, $report ) {
		...
		$message .= sprintf(
			...
			__( 'Un %s a été signalé comme %s: %s', $this->plugin_slug ) . "<br><br>",
		...
		$message .= sprintf( __( 'Signalé par: %s', $this->plugin_slug ) . "<br>", $reported_by );
		...
		$this->send_notification( $message, __( 'Signalé ' ) . $post->post_type, $post_id );
