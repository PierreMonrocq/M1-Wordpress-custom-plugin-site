...
class bbPressModToolsPlugin_Report extends bbPressModToolsPlugin {
	...
	public function render_content_reports( $content, $post_id ) {
		...
		if ( $this->is_reported( $post_id ) && $this->user_can_manage_reports() ) {
			...
			foreach ( $report_types as $type => $count ) {
				...
				$notices .= '<div class="bbp-mt-template-notice">' . __( $count . ' utilisateurs ont signalé ceci comme ' . $type . '.', $this->plugin_slug ) . '</div>';
	...
	public function add_report_link( $links, $post_id ) {
		...
		if ( is_user_logged_in() && get_option( '_bbp_report_post' ) ) {
			...
			array_push( $links,
				...
				<span class="bbp-report-type">
					...
					<option>' . __( 'Raison du signalement', $this->plugin_slug ) . '</option>

					<option>' . __( 'Spam', $this->plugin_slug ) . '</option>

					<option>' . __( 'Publicité', $this->plugin_slug ) . '</option>

					<option>' . __( 'Harcèlement', $this->plugin_slug ) . '</option>

					<option>' . __( 'Contenu inapproprié', $this->plugin_slug ) . '</option>
		...
		if ( $this->is_reported( $post_id ) && $this->user_can_manage_reports() ) {
			...
			array_push( $links, '<a href="' . $nonce_url . '" class="bbp-reply-edit-link">' . __( 'Effacer le signalement', $this->plugin_slug ) . '</a>' );
	...
	*/
	...
	public function handle_report_post() {
		...
		_e( 'Votre signalement va être étudié par notre équipe de modération.', $this->plugin_slug );
